 ...
 * 使用极线搜索 + NCC 匹配的方式，与《十四讲》的 13.2 节对应
 ***********************************************/
...
const int num_frames = 29;    // 选取多少张图片计算ref 图片的深度
//const int total_num_frames = 200;    //总的图片数量

bool getDepthEstimation(const string& path, const int& index, const int& total_num_frames)
{
    // 从数据集读取数据
    vector<string> color_image_files;
    vector<SE3> poses_TWC;
    
    bool ret = readDatasetFiles(path, color_image_files, poses_TWC, index );
    if ( ret==false )
    {
    ...
    for (int i; i<color_image_files.size(); i++)
    {
        Mat ref = imread( color_image_files[i], 0 );                // gray-scale image
        SE3 pose_ref_TWC = poses_TWC[i];
        double init_depth   = 3.0;    // 深度初始值
        double init_cov2    = 3.0;    // 方差初始值
        Mat depth( height, width, CV_64F, init_depth );             // 深度图
        Mat depth_cov( height, width, CV_64F, init_cov2 );          // 深度图方差
        
        for ( int j=1; j<color_image_files.size(); j++ )
        {
            if (j==i) continue;
            cout<<"*** loop "<<j<<" ***"<<endl;
            Mat curr = imread( color_image_files[j], 0 );
            if (curr.data == nullptr) continue;
            SE3 pose_curr_TWC = poses_TWC[j];
        ...
        // save depth
        cout<<"estimation returns, saving depth data ..."<<endl;
        string file_name = color_image_files[i].substr(0, color_image_files[i].length()-3);
        saveDepthEstimation(file_name, depth);
        
        cout<<"estimation returns, saving depth map ..."<<endl;
        imwrite( file_name+".png", depth );
        cout << "The" << index << "map was done." << endl;
    }
...
bool readDatasetFiles(
                      ...
                      const int index,
                      const int& total_num_frames
                      )
{
    int start_frame;
    if (index < (num_frames/2)){
        start_frame = 0;
    }
    else if (index > (total_num_frames-num_frames/2)){
        start_frame = total_num_frames - num_frames;
    }
    else{
        start_frame = index - num_frames/2;
    }
    ...
    while ( !fin.eof() )
    {
        if (lcount < start_frame){
            lcount++;
            continue;
        }
        if (lcount > start_frame+num_frames){
            break;
        }
...
bool saveDepthEstimation(const string& file_name, Mat& depth){
    ofstream fout("depth/"+file_name+".txt");
